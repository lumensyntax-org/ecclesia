...
2. **It is applied in a narrative or work.** The understanding takes form in something specific — a story, a painting, an entry, a teaching, an act. Understanding without application is suspended; it does not mediate because it has not yet entered the medium.
...
What the third condition points to is the producer's actual disposition with respect to the source-position. Like the minister's interior intention, this is not measurable from outside — only its fruit is. The compositional or technical content of any mediating work is necessary but insufficient; what completes it as faithful mediation is that disposition.
...
The age, authority, or reception of a source does not establish the strength of the project's cross-domain mapping. The **STRONG** rating applies to the underlying concern as it is attested across disciplines and centuries; the three-condition compression, the convergence claim, and the recursion clause are the **project's proposal and interpretation**. The test is **applied and evaluated** concretely — but retrospectively, by what a production does in its receivers over time — so it is evaluable rather than falsifiable in the strict sense. This entry does not certify that the project has satisfied its own standard; that is observable only in fruit.
