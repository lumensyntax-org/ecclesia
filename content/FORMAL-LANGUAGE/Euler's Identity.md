...
properties: [alignment, proportion, non_fabrication]
strength: STRONG
...
Euler's identity, e^(i*pi) + 1 = 0, connects five fundamental constants from different branches of mathematics: e (analysis), i (algebra), pi (geometry), 1 (arithmetic), 0 (the additive identity). The identity is a special case of Euler's formula e^(ix) = cos(x) + i*sin(x), evaluated at x = pi. Five constants arising from distinct mathematical domains converge in a single equation. Their connection is not imposed -- it is discovered.
